...
export class DepogirisislemiPage extends BasePage {
  private readonly lookupMap = new Map<string, { input: string; btn: string; value: string }>([
    ...
    ['GirisDepoKod', { input: '#MCC_edtGirisDepoKodu_NE_t', btn: '#MCC_edtGirisDepoKodu_NE_b0 > img', value: '18' }],
  ...
  private readonly S_GIRISDEPOKOD_INPUT = '#MCC_edtGirisDepoKodu_NE_t';
  private readonly S_GIRISDEPOKOD_BTN = '#MCC_edtGirisDepoKodu_NE_b0 > img';
  ...
  async fillHeaderLookups(): Promise<void> {
    ...
    await this.selectLookupByKey('GirisDepoKod');
  }
  ...
  async editDetailQuantity(quantity = '99.00'): Promise<void> {
    const frame = await this.resolveFormFrame(10000);
    await this.ensureEditorVisibleAfterTriggerInFrame(
