...
export class AlisirsaliyesiPage extends BasePage {
  private readonly lookupMap = new Map<string, { input: string; btn: string; value: string }>([
    ...
    ['MustKod', { input: '#MCC_edtMustKod_TE_t', btn: '#MCC_edtMustKod_TE_b0 > img', value: 'TEST' }],
    ['UrunKod', { input: '#DCC_DetGrd_RT_edtDetUrunKod_TE_t', btn: '#DCC_DetGrd_RT_edtDetUrunKod_TE_b0 > img', value: 'TS1_118_1' }],
  ]);

  private readonly S_MENU_0 = 'a > span[data-menu-kod="1202"]';
  private readonly S_MENU_1 = 'a > span[data-menu-kod="1216"]';
  private readonly S_MENU_2 = 'a > span[data-menu-kod="1218"]';
  private readonly S_LIST_HEADER = '#ListGrid_R0 > th';
  private readonly S_NEW_ACTION = '#NvgxToolbar_Item_1 > span';
  private readonly S_EDIT_ROW = '#ListGrid_R0C0';
  ...
  private readonly S_EDITDETAILQUANTITY_EDITOR = '#DCC_DetGrd_RT_edtDetMik_NBE_t';
  private readonly S_EDITDETAILQUANTITY_QTY = '#DCC_DetGrd_RT_edtDetMik_NBE_t';
  ...
  private async resolveListFrame(timeout = 15000): Promise<Frame> {
    this.page = this.getActivePage([/\/Interface\/MobilSatisDagitim\/IrsaliyeAlisExtDMLList\.aspx/i]);
    return this.resolveFrameByAnyVisibleMarker([this.S_LIST_HEADER], timeout, [/\/Interface\/MobilSatisDagitim\/IrsaliyeAlisExtDMLList\.aspx/i]);
  }

  private async resolveFormFrame(timeout = 15000): Promise<Frame> {
    this.page = this.getActivePage([/\/Interface\/MobilSatisDagitim\/IrsaliyeAlisExtDML\.aspx/i]);
    return this.resolveFrameByAnyVisibleMarker(['#MCC_edtDistKod_TE_t'], timeout, [/\/Interface\/MobilSatisDagitim\/IrsaliyeAlisExtDML\.aspx/i]);
  }
  ...
  async editDetailQuantity(quantity = '102'): Promise<void> {
    const frame = await this.resolveFormFrame(10000);
    await this.ensureEditorVisibleAfterTriggerInFrame(
