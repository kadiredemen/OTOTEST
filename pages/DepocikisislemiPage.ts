...
export class DepocikisislemiPage extends BasePage {
  private readonly lookupMap = new Map<string, { input: string; btn: string; value: string }>([
    ...
    ['CikisDepoKod', { input: '#MCC_edtCikisDepoKodu_NE_t', btn: '#MCC_edtCikisDepoKodu_NE_b0 > img', value: '18' }],
    ['UrunKod', { input: '#DCC_DetGrd_RT_edtDetUrunKod_TE_t', btn: '#DCC_DetGrd_RT_edtDetUrunKod_TE_b0 > img', value: 'TS1_118_2' }],
  ]);

  private readonly S_MENU_0 = 'a > span[data-menu-kod="1295"]';
  private readonly S_MENU_1 = 'a > span[data-menu-kod="1308"]';
  private readonly S_MENU_2 = 'a > span[data-menu-kod="1310"]';
  private readonly S_LIST_HEADER = '#ListGrid_R0 > th';
  private readonly S_NEW_ACTION = '#NvgxToolbar_Item_1 > span';
  private readonly S_EDIT_ROW = '#ListGrid_R2C0';
  private readonly S_EDIT_ACTION = '#NvgxToolbar_Item_2 > span';
  private readonly S_DISTKOD_INPUT = '#MCC_edtDistKod_TE_t';
  private readonly S_DISTKOD_BTN = '#MCC_edtDistKod_TE_b0 > img';
  private readonly S_CIKISDEPOKOD_INPUT = '#MCC_edtCikisDepoKodu_NE_t';
  private readonly S_CIKISDEPOKOD_BTN = '#MCC_edtCikisDepoKodu_NE_b0 > img';
  private readonly S_ADDDETAIL_TRIGGER = '#DCCxGridNvgxToolbar_Item_1 > span > i';
  private readonly S_ADDDETAIL_EDITOR = '#DCC_DetGrd_RT_edtDetUrunKod_TE_t';
  private readonly S_ADDDETAIL_QTY = '#DCC_DetGrd_RT_edtDetMiktar_NBE_t';
  private readonly S_ADDDETAIL_OK = '#DCC_DetGrd_RT_DCC_DetGrdBtnOK';
  private readonly S_ADDDETAIL_ROW = '#DCC_DetGrd_R0 > th';
  private readonly S_EDITDETAILQUANTITY_TRIGGER = '#DCCxGridNvgxToolbar_Item_2 > span';
  private readonly S_EDITDETAILQUANTITY_EDITOR = '#DCC_DetGrd_RT_edtDetMiktar_NBE_t';
  private readonly S_EDITDETAILQUANTITY_QTY = '#DCC_DetGrd_RT_edtDetMiktar_NBE_t';
  private readonly S_EDITDETAILQUANTITY_OK = '#DCC_DetGrd_RT_DCC_DetGrdBtnOK';
  private readonly S_EDITDETAILQUANTITY_ROW = '#DCC_DetGrd_R0 > th';
  private readonly S_SAVE = '#MainNvgxToolbar_Item_0 > span';
  private readonly S_CLOSE = '#MainNvgxToolbar_Item_2 > span > i';
  ...
  private async resolveFormFrame(timeout = 15000): Promise<Frame> {
    ...
    return this.resolveFrameByAnyVisibleMarker(['#MCC_edtDistKod_TE_t'], timeout, [/\/Interface\/Lists\/ListCommonDML\.aspx/i]);
  }
  ...
  async fillHeaderLookups(): Promise<void> {
    ...
    await this.selectLookupByKey('CikisDepoKod');
  }
  ...
  async addDetail(quantity = '100'): Promise<void> {
    ...
    await this.commitDetailRowByProofsInFrame(frame, {
      ...
      proofSelectors: [
        ...
        { selector: '#DCC_DetGrd_RT_edtDetMiktar_NBE_t', state: 'hidden' as const },
        { selector: '#DCC_DetGrd_RT_TBL', state: 'hidden' as const },
      ],
    });
  }

  async editDetailQuantity(quantity = '3.00'): Promise<void> {
    const frame = await this.resolveFormFrame(10000);
    await this.ensureEditorVisibleAfterTriggerInFrame(
      frame, this.S_EDITDETAILQUANTITY_TRIGGER, this.S_EDITDETAILQUANTITY_EDITOR, { attempts: 3, timeoutPerAttempt: 5000 }
    );

    await this.fillRobustInFrame(frame, this.S_EDITDETAILQUANTITY_QTY, quantity);
    await frame.locator(this.S_EDITDETAILQUANTITY_QTY).first().press('Tab');
    await this.commitDetailRowByProofsInFrame(frame, {
      okButtonSelector: this.S_EDITDETAILQUANTITY_OK,
      rowProofSelector: this.S_EDITDETAILQUANTITY_ROW,
      retries: 2,
      timeout: 10000,
      proofSelectors: [
        { selector: '#DCC_DetGrd_R0 > th', state: 'visible' as const },
